...
*** e:mode-key-def
#+begin_src emacs-lisp
(defmacro e:major-mode-key-def (mode key def &rest bindings)
  (declare (indent defun))
  `(e:mode-key-def :major-modes ,mode ,key ,def ,@bindings))
(defmacro e:minor-mode-key-def (mode key def &rest bindings)
  (declare (indent defun))
  `(e:mode-key-def :minor-modes ,mode ,key ,def ,@bindings))
(defmacro e:mode-key-def (mode-key mode key def &rest bindings)
  (declare (indent defun))
...
(progn
  (defvar last-gc-elapsed 0.0)
  (add-hook 'post-gc-hook
            (defun e:post-gc-message ()
              (message "GC! > %.4f[sec]" (- gc-elapsed last-gc-elapsed))
              (setq last-gc-elapsed gc-elapsed))))
#+end_src
** comp
#+begin_src emacs-lisp
...
(e:deffered-config! display-line-numbers
  :hook ((find-file-hook . e:display-line-numbers-mode-on)
         ...
         (org-mode-hook  . e:display-line-numbers-mode-on))
  :defer-config
...
(e:deffered-config! whitespace
  :hook ((find-file-hook . e:whitespace-mode-on)
         (prog-mode-hook . e:whitespace-mode-on))
  :defer-config
...
** functions
#+begin_src emacs-lisp
(e:deffered-config! skk-functions
...
(e:deffered-config! skk-server
  ...
  (and (fboundp 'e:prodigy-yaskkserv2)
...
(e:package-config! cape
  :defun (e:capf-functions
          (cape-tabnine . company-tabnine))
...
(e:package-config! consult
  ...
  :commands (consult-line-dwim consult-ripgrep-dwim consult-ripgrep-cwd consult-ripgrep-cwd-dwim)
...
** パッケージのインストール待ち
#+begin_src emacs-lisp
(eval-when-compile (eval '(elpaca-wait)))
