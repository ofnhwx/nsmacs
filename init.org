...
*** NixOS
#+begin_src emacs-lisp
(e:if! (--find (s-match "^/nix/.+?/jinx" it) load-path)
  (setopt load-path (--reject (s-match "lib/jinx$" it) load-path))
  (e:debug-log "done: NixOS config."))
#+end_src
*** Mac
#+begin_src emacs-lisp
...
(e:deferred-config! claude-code-ide
  :advice (:filter-return claude-code-ide--didplay-buffer-in-side-window ad:claude-code-ide--display-buffer-in-side-window@setup)
  :config
  (claude-code-ide-emacs-tools-setup)
  (setopt claude-code-ide-focus-on-open nil)
  (setopt claude-code-ide-focus-claude-after-ediff nil)
  (setopt claude-code-ide-terminal-backend 'vterm)
  (setopt claude-code-ide-use-ide-diff nil))
#+end_src
** denote
 denote
#+begin_src emacs-lisp
...
(e:deferred-config! magit
  ...
  (when (require 'forge nil t)
    ...
    (when (require 'forge-review-overlay nil t)
...
(e:key-def application a
  ...
  "Cw" 'chezmoi-write
  "c" '(:ignore t :wk "claude-code-ide")
  "c ESC" 'claude-code-ide-send-escape
  "cc" 'claude-code-ide-continue
  "ci" 'claude-code-ide-insert-at-mentioned
  ...
  "cw" 'claude-code-ide-switch-to-buffer
