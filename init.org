...
(when init-file-debug
  (message "[%f] loading: %s." (float-time (time-since before-init-time)) "early-init.el"))
#+end_src
*** init.el
...
;;; init.el
(when init-file-debug
  (message "[%f] loading: %s." (float-time (time-since before-init-time)) "init.el"))
#+end_src
** early-init.el
*** 起動時間短縮のための設定
#+begin_src emacs-lisp :tangle early-init.el
...
(borg-initialize)
(when init-file-debug
  (message "[%f] done: %s." (float-time (time-since before-init-time)) "borg initialized"))
#+end_src
** ライブラリ
#+begin_src emacs-lisp
(require 'nsmacs-macros)
(require 'nsmacs-deferred)
...
(e:debug-log "done: load libraries.")
#+end_src
** 環境設定
*** Env
#+begin_src emacs-lisp
(e:load-env-file "~/.config/env")
...
(load-theme (if (display-graphic-p) 'modus-vivendi-tinted 'modus-vivendi) :no-confirm)
(e:debug-log "done: load theme.")
#+end_src
*** Font
#+begin_src emacs-lisp
...
(e:debug-log "done: set font.")
#+end_src
*** Japanese
#+begin_src emacs-lisp
(set-language-environment "Japanese")
(e:debug-log "done: set language environment.")
#+end_src
*** Encoding
#+begin_src emacs-lisp
...
(e:debug-log "done: set coding system.")
#+end_src
*** Locale
#+begin_src emacs-lisp
(let ((value "ja_JP.UTF-8"))
  (setenv "LANG" value)
  (setenv "LC_ALL" value))
(e:debug-log "done: set locale.")
#+end_src
* Config: Emacs
** Alias
#+begin_src emacs-lisp
(defalias 'exit 'save-buffers-kill-terminal)
(e:debug-log "done: set alias.")
#+end_src
** startup
#+begin_src emacs-lisp
...
(e:debug-log "done: startup config.")
#+end_src
** C Sources
#+begin_src emacs-lisp
...
(e:debug-log "done: set C sources config.")
#+end_src
** 個人設定
#+begin_src emacs-lisp
(e:deferred-config! load-private-config
  :config
    ...
    (condition-case err
      ...
      (error
       (display-warning 'initialization
                        (format "Failed to load private-config: %s" (error-message-string err))
                        :warning)))))
#+end_src
** auth-source
#+begin_src emacs-lisp
...
(e:deferred-config! ediff
  ...
  (add-hook 'ediff-before-setup-hook #'e:ediff-save-window-configuration)
  (add-hook 'ediff-quit-hook #'e:ediff-restore-window-configuration))
#+end_src
** emacs-lock
#+begin_src emacs-lisp
...
** project
#+begin_src emacs-lisp
(e:deferred-config! project
  :advice (:before project-prompt-project-dir ad:project-prompt-project-dir@update)
  :defer-config
...
(e:deferred-config! vc-hooks
  :defer-config
  (defvar original-vc-ignore-dir-regexp vc-ignore-dir-regexp)
  (setopt vc-ignore-dir-regexp
          (format "\\(%s\\)\\|\\(%s\\)" original-vc-ignore-dir-regexp tramp-file-name-regexp))
...
** evil-ghostel
#+begin_src emacs-lisp
(e:deferred-config! evil-ghostel
  :after (evil)
...
(e:deferred-config! skk-server
  ...
  (setopt yaskkserv2-dictionary (f-expand "~/sync/drive/share/dictionary.yaskkserv2"))
...
(e:deferred-config! corfu
  ...
  :hook ((corfu-mode-hook . corfu-echo-mode)
         ...
         (minibuffer-setup-hook . e:corfu-enable-always-in-minibuffer))
  :init
  (setopt corfu-auto t)
...
** evil-org-agenda
#+begin_src emacs-lisp
(e:deferred-config! evil-org-agenda
  :after (evil-org)
  :require t
...
(e:deferred-config! apheleia
  ...
  (setopt apheleia-skip-functions '(buffer-modified-p))
  ;; formatters
  (setf (alist-get 'rubocop apheleia-formatters)
        '((if (e:bundle-exists "rubocop") '("bundle" "exec" "rubocop") "rubocop")
          "--server" file "--autocorrect" "--stderr" "--format" "quiet" "--fail-level" "fatal"))
  (setf (alist-get 'haml-lint apheleia-formatters)
        '((if (e:bundle-exists "haml_lint") '("bundle" "exec" "haml-lint") "haml-lint")
          file "--auto-correct" "--stderr" "--fail-fast" "--fail-level" "error"))
  ;; mode-alist
  (setf (alist-get 'ruby-ts-mode apheleia-mode-alist) '(rubocop))
  (setf (alist-get 'haml-mode apheleia-mode-alist) '(haml-lint)))
#+end_src
** avy
#+begin_src emacs-lisp
...
** flash
#+begin_src emacs-lisp
(e:deferred-config! flash
  :after (evil)
  :config
  (general-def 'normal "s" 'flash-evil-jump)
  (general-def 'visual "x" 'flash-evil-jump))
#+end_src
** chezmoi
#+begin_src emacs-lisp
(e:deferred-config! chezmoi
  :commands (chezmoi-diff
             ...
             chezmoi-find-x
             chezmoi-open-other
             chezmoi-open-other-x
             chezmoi-sync-files
             chezmoi-sync-files-x
             chezmoi-template-buffer-display
             chezmoi-template-buffer-display-x
             chezmoi-write
             chezmoi-write-x)
  ...
  (--each '(chezmoi-diff
            ...
            chezmoi-template-buffer-display
            chezmoi-write)
    (let ((name (intern (format "%s-x" it)))
          (orig it))
      (defalias name
        (lambda ()
          (interactive)
          (dlet ((chezmoi-command "x"))
            (call-interactively orig)))))))
#+end_src
** conflict-buttons
#+begin_src emacs-lisp
(e:deferred-config! conflict-buttons
  :hook (smerge-mode-hook . conflict-buttons-mode))
#+end_src
** denote
 denote
#+begin_src emacs-lisp
...
(e:deferred-config! denote-menu
  :defer-config
  (setopt denote-menu-show-file-signature t)
  (setopt denote-menu-signature-column-width 25)
  (setopt denote-menu-title-column-width 50)
  (setopt denote-menu-keywords-column-width 28))
#+end_src
** diff-hl
#+begin_src emacs-lisp
(e:deferred-config! diff-hl
  :defun (diff-hl-magit-pre-refresh diff-hl-magit-post-refresh)
  :config
...
** emacs-cdp
#+begin_src emacs-lisp
(e:deferred-config! emacs-cdp
  :defvar (emacs-cdp-mode)
  :defun (evil-emacs-state evil-normal-state)
  :defer-config
  (setopt emacs-cdp-profile-directory (f-expand "~/.config/chrome-debug"))
...
(e:deferred-config! magit
  :advice ((:before   magit-read-repository  ad:magit-update-repository-directories)
           (:before   magit-repolist-setup   ad:magit-update-repository-directories)
           (:override magit-repos-alist      ad:magit-repos-alist@override))
  :defun (magit-add-section-hook)
  :defvar (magit-git-environment)
  :defer-config
  (setopt magit-delete-by-moving-to-trash nil)
  (setopt magit-diff-highlight-indentation '(("" . tabs)))
  (setopt magit-diff-highlight-trailing t)
  (setopt magit-diff-paint-whitespace t)
  ...
  ;; pre-commit hook などの CLI ツールに非対話モードで出力させる
  ;; (add-to-list 'magit-git-environment "CI=1")
  (add-to-list 'magit-git-environment "FORCE_COLOR=0")
...
*** forge
#+begin_src emacs-lisp
(e:deferred-config! forge
  :after (magit)
  :require t
  :advice ((:filter-return forge--list-topics-2 ad:forge--list-topics-2@multi-author)
           (:filter-return forge--list-topics   ad:forge--list-topics@draft-last))
  :config
  ...
  (set-face-attribute 'forge-issue-open nil :foreground 'unspecified)
  (set-face-attribute 'forge-issue-completed nil :foreground 'unspecified :inherit 'forge-dimmed)
  (set-face-attribute 'forge-pullreq-draft nil :foreground "#525252" :inherit nil)
  ...
  (e:setup-forge-multi-author-filter))
#+end_src
*** forge-review-overlay
#+begin_src emacs-lisp
(e:deferred-config! forge-review-overlay
  :after (forge)
  :require t
  :config
  (setopt forge-review-overlay-ignored-reviewers
          '("claude"
            "copilot-pull-request-reviewer"
            "gemini-code-assist"
            "github-actions"))
  (setopt forge-review-overlay-highlighted-reviewer
          (e:shell-output! "gh api user -q .login"))
  (add-hook 'magit-status-mode-hook #'forge-review-overlay-mode))
#+end_src
*** forge-review-section
#+begin_src emacs-lisp
(e:deferred-config! forge-review-section
  :after (forge)
  :require t
  :config
  (forge-review-section-mode 1))
#+end_src
*** magit-delta
#+begin_src emacs-lisp
...
*** magit-status
#+begin_src emacs-lisp
(e:deferred-config! magit-status
  :defer-config
  ;; magit のデフォルトは --ignore-submodules=none で diff.ignoreSubmodules を上書きしてしまう
  (put 'magit-status-mode 'magit-diff-default-arguments
       '("--no-ext-diff" "--ignore-submodules=dirty")))
#+end_src
*** magit-submodule-cache
#+begin_src emacs-lisp
(e:deferred-config! magit-submodule-cache
  :after (magit)
  :config
  (magit-submodule-cache-setup))
#+end_src
** mise
#+begin_src emacs-lisp
...
** popterm
#+begin_src emacs-lisp
(e:deferred-config! popterm
  :defer-config
  (setopt popterm-display-method 'window)
  (setopt popterm-window-height-ratio 0.6)
...
(e:deferred-config! prodigy
  :advice (:around prodigy-start-service ad:prodigy-start-service@with-pty)
...
(e:deferred-config! shackle
  ...
  (setopt shackle-rules
          '(;;
            ...
            (helpful-mode         :align bottom :ratio 0.35 :select t)
            ;;
            ...
            ("*project-rails-compilation*"    :align bottom :ratio 0.35)
...
** sticky-window
#+begin_src emacs-lisp
(e:deferred-config! sticky-window
  :defer-config
...
** treesit-fold
#+begin_src emacs-lisp
(e:deferred-config! treesit-fold
  :config
...
** vulpea
#+begin_src emacs-lisp
(e:deferred-config! vulpea
  :defer-config
  (e:var! vulpea-db-location "vulpea/vulpea.db"))
#+end_src
** wakatime-mode
#+begin_src emacs-lisp
...
(e:deferred-config! haml-mode
  :hook ((haml-mode-hook . e:setup-haml-mode)
         ...
         (haml-mode-hook . indent-bars-mode)))
#+end_src
*** project-rails
#+begin_src emacs-lisp
(e:deferred-config! project-rails
  :defun (project-rails-global-mode)
  :defvar (project-rails-fixture-dirs)
  :advice ((:override project-rails--choices ad:project-rails--choices@override))
  :init
  (project-rails-global-mode 1)
  :config
  (e:setup-project-rails-views-re)
  (e:var! rake-cache-file "rake.cache")
  (setopt rake-completion-system 'completing-read-default)
  (e:setup-project-rails-annotation controller "app/controllers" "*")
  (e:setup-project-rails-annotation environment "config")
  (e:setup-project-rails-annotation fixture project-rails-fixture-dirs)
  (e:setup-project-rails-annotation helper "app/helpers" "_helper")
  (e:setup-project-rails-annotation initializer "config/initializers")
  (e:setup-project-rails-annotation job "app/jobs" "*")
  (e:setup-project-rails-annotation layout "app/views/layouts")
  (e:setup-project-rails-annotation lib "lib")
  (e:setup-project-rails-annotation locale "config/locales")
  (e:setup-project-rails-annotation log "log")
  (e:setup-project-rails-annotation mailer "app/mailers")
  (e:setup-project-rails-annotation migration "db/migrate")
  (e:setup-project-rails-annotation model "app/models")
  (e:setup-project-rails-annotation spec "spec" "_spec")
  (e:setup-project-rails-annotation validator "app/validators" "*")
  (e:setup-project-rails-annotation view "app/views")
  (e:setup-project-rails-annotation decorator "app/decorators" nil e:project-rails-find-decorator)
  (e:setup-project-rails-annotation form "app/forms" nil e:project-rails-find-form)
  (e:setup-project-rails-annotation view-components "app/components" nil e:project-rails-find-view-components)
  (e:setup-project-rails-annotation graphql "app/graphql" nil e:project-rails-find-graphql)
  (e:setup-project-rails-annotation route "config" nil e:project-rails-find-route)
  (e:minor-mode-key-def project-rails-mode
    "f" "rails"
    "f:" '("rake" . rake)
    "fc" "generate/destroy"
    "fcc" '("generate" . project-rails-generate)
    "fcd" '("destroy" . project-rails-destroy)
    "ff" "find"
    "ff@" '("mailer" . project-rails-find-mailer)
    "ffF" '("feature" . project-rails-find-feature)
    "ffR" '("rake task" . rake-find-task)
    "ffV" '("view component" . e:project-rails-find-view-components)
    "ffa" '("locale" . project-rails-find-locale)
    "ffb" '("job" . project-rails-find-job)
    "ffc" '("controller" . project-rails-find-controller)
    "ffd" '("decorator" . e:project-rails-find-decorator)
    "ffe" '("environment" . project-rails-find-environment)
    "fff" '("form" . e:project-rails-find-form)
    "ffg" '("graphql" . e:project-rails-find-graphql)
    "ffh" '("helper" . project-rails-find-helper)
    "ffi" '("initializer" . project-rails-find-initializer)
    "ffj" '("javascript" . project-rails-find-javascript)
    "ffl" '("lib" . project-rails-find-lib)
    "ffm" '("model" . project-rails-find-model)
    "ffn" '("migration" . project-rails-find-migration)
    "ffo" '("log" . project-rails-find-log)
    "ffp" '("spec" . project-rails-find-spec)
    "ffr" '("route" . e:project-rails-find-route)
    "ffs" '("stylesheet" . project-rails-find-stylesheet)
    "fft" '("test" . project-rails-find-test)
    "ffu" '("fixture" . project-rails-find-fixture)
    "ffv" '("view" . project-rails-find-view)
    "ffy" '("layout" . project-rails-find-layout)
    "fg" "goto"
    "fgc" '("controller" . project-rails-find-current-controller)
    "fgd" '("schema" . project-rails-goto-schema)
    "fge" '("seeds" . project-rails-goto-seeds)
    "fgg" '("gemfile" . project-rails-goto-gemfile)
    "fgh" '("helper" . project-rails-find-current-helper)
    "fgm" '("model" . project-rails-find-current-model)
    "fgn" '("migration" . project-rails-find-current-migration)
    "fgp" '("spec" . project-rails-find-current-spec)
    "fgr" '("routes" . project-rails-goto-routes)
    "fgt" '("test" . project-rails-find-current-test)
    "fgu" '("fixture" . project-rails-find-current-fixture)
    "fgv" '("view" . project-rails-find-current-view)
    "fgz" '("helper" . project-rails-goto-spec-helper)
    "i" "insert"
...
#+begin_src emacs-lisp
(declare-function general-simulate-SPC "general")
...
(e:key-def application a
  ...
  "cD" 'chezmoi-diff-x
  "cF" 'chezmoi-find-x
  "cO" 'chezmoi-open-other-x
  "cS" 'chezmoi-sync-files-x
  "cT" 'chezmoi-template-buffer-display-x
  "cW" 'chezmoi-write-x
  "cd" 'chezmoi-diff
  "cf" 'chezmoi-find
  ...
  "g" '(:ignore t :wk "Chrome(CDP)")
  "gg" 'emacs-cdp-mode
  "gi" 'emacs-cdp-insert-text
  "gn" 'emacs-cdp-navigate
  "gr" 'emacs-cdp-reload-page
  "gs" 'emacs-cdp-select-tab
  "gt" 'emacs-cdp-new-tab
  "o" '(:ignore t :wk "org")
  "oa" 'org-agenda-list
  "oc" 'org-capture
  "ol" 'org-store-link
...
(e:key-def file f
  ...
  "el" '(e:file/find-lib-directory :wk "nsmacs-lib")
...
(e:key-def project p
  ...
  "c" 'project-compile
  "d" 'project-dired
  "e" 'e:project-edit-dir-locals
  "f" 'project-find-file
  "g" 'project-find-regexp
  "k" 'project-kill-buffers
  "p" 'project-switch-project
  "r" 'consult-project-buffer
  "v" 'project-vc-dir)
#+end_src
*** [q] quit
#+begin_src emacs-lisp
...
** 起動時間高速化のための設定を元に戻す
#+begin_src emacs-lisp
(declare-function e:restore-startup-config "early-init")
...
(e:deferred-config! complete-start-process
  :config
  (run-with-timer 1.0 nil #'e:deferred-config-stats)
  (when init-file-debug
